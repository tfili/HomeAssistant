...
$ha_timezone = 'America/New_York'; # Supported Timezones https://www.php.net/manual/en/timezones.php
...
if(is_file($image_latest)&&!isset($_GET['last'])) {
  header("Content-Type: image/png");
  echo file_get_contents($image_latest);
  die();
}
...
imagepng($dest);
if(isset($_GET['last'])) {
  imagepng($dest, $image_latest);
  imagepng($dest, $image_basename.$date.".png");
}
imagedestroy($dest);
imagedestroy($roomba);
imagedestroy($roomba_stuck);
